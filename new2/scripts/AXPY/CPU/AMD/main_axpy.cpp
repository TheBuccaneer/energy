...
int main(int argc, char** argv) {
    try {
        const bench::Options options = bench::parse_options(argc, argv, "axpy_amd.csv");
        ...
        for (const Config config : configs) {
            ++config_number;
            std::cout << "[AXPY] preparing config " << config_number << '/' << configs.size()
            ...
            std::cout << "[AXPY] calibrated config " << config_number << '/' << configs.size()
