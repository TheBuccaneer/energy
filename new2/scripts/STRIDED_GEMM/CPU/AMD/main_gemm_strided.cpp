...
int main(int argc, char** argv) {
    try {
        const bench::Options options = bench::parse_options(argc, argv, "gemm_strided_amd.csv");
        ...
        for (const Config config : configs) {
            ++config_number;
            std::cout << "[STRIDED_GEMM] preparing config " << config_number << '/' << configs.size()
                      << ": N=" << config.n << " threads=" << config.threads << std::endl;
            ...
            std::cout << "[STRIDED_GEMM] calibrated config " << config_number << '/' << configs.size()
                      << ": batches=" << batches << std::endl;
