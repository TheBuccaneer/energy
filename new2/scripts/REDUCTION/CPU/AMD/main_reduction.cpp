...
int main(int argc, char** argv) {
    try {
        const bench::Options options = bench::parse_options(argc, argv, "reduction_amd.csv");
        ...
        for (const Config config : configs) {
            ++config_number;
            std::cout << "[REDUCTION] preparing config " << config_number << '/' << configs.size()
            ...
            std::cout << "[REDUCTION] calibrated config " << config_number << '/' << configs.size()
