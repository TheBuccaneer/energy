...
int main(int argc, char** argv) {
    try {
        const bench::Options options = bench::parse_options(argc, argv, "stream_amd.csv");
        ...
        for (const Config config : configs) {
            ++config_number;
            std::cout << "[STREAM] preparing config " << config_number << '/' << configs.size()
            ...
            std::cout << "[STREAM] calibrated config " << config_number << '/' << configs.size()
