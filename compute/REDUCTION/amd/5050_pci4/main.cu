...
void writeCSVRow(std::ofstream& file, const std::string& host,
    ...
    // energy_j = primary energy for this row (tidy data principle)
    double energy_j = is_kernel_mode ? energy_kernel_j : energy_e2e_j;
    
    // Compute power separately for each mode (always, for separate columns)
    ...
    // avg_power_w = primary power for this row (tidy: energy_j / seconds_wall)
