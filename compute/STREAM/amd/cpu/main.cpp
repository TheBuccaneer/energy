...
// Compile: g++ -O3 -march=native -fopenmp -std=c++17 -o stream_cpu main.cpp
...
// AMD-robust: Sum all L3 slices across all sockets
size_t getTotalCPULLCSize() {
    const char* base = "/sys/devices/system/cpu";
    std::map<int, size_t> llc_per_socket;  // socket_id -> total L3
    std::set<std::string> counted_slices;  // FIX: Non-static to avoid multi-call bug

    DIR* dir = opendir(base);
    if (!dir) {
        // Fallback: assume 32 MB LLC
        return 32 * 1024 * 1024;
    }
    ...
    while ((entry = readdir(dir)) != nullptr) {
        ...
        // Get physical package id for this CPU
        std::ifstream pkg_file(cpu_path + "/topology/physical_package_id");
        int pkg_id = 0;
        if (pkg_file.is_open()) {
            pkg_file >> pkg_id;
        } else {
        ...
        // Iterate through cache indices to find L3 (level=3, type=Unified)
        for (int idx = 0; idx < 10; idx++) {
            std::string cache_base = cpu_path + "/cache/index" + std::to_string(idx);

            // Check if this cache exists
            std::ifstream level_file(cache_base + "/level");
            if (!level_file.is_open()) continue;

            int level = 0;
            level_file >> level;
            if (level != 3) continue;  // Only L3

            // Check type (must be Unified or Data)
            std::ifstream type_file(cache_base + "/type");
            std::string type;
            ...
            // Read size
            std::ifstream size_file(cache_base + "/size");
            if (!size_file.is_open()) continue;
            ...
            // Read shared_cpu_list to avoid double-counting
            std::ifstream shared_file(cache_base + "/shared_cpu_list");
            std::string shared_cpus;
            if (shared_file.is_open()) {
                std::getline(shared_file, shared_cpus);
            }

            // Only count this L3 slice once per unique shared_cpu_list
            // (on Zen, each CCD has one L3 shared by its cores)
            std::string slice_key = std::to_string(pkg_id) + ":" + shared_cpus;

            if (counted_slices.find(slice_key) == counted_slices.end()) {
                llc_per_socket[pkg_id] += bytes;
                counted_slices.insert(slice_key);
            }

            break;  // Found L3 for this CPU, move to next CPU
        }
    }
    closedir(dir);

    // Sum across all sockets
    size_t total_llc = 0;
    for (const auto& kv : llc_per_socket) {
        total_llc += kv.second;
    }
    ...
    if (total_llc == 0) {
        ...
        total_llc = num_sockets * 32 * 1024 * 1024;  // 32 MB per socket fallback
    }
                                   ...
                                   for (size_t i = 0; i < N; i++) {
                                       a[i] = b[i] + q * c[i];
                                                 ...
                                                 void writeCSVRow(std::ofstream& file,
                                                     ...
                                                     // Build notes with energy status
                                                     ...
                                                     // FIX: Proper string formatting with helper lambda
                                                     ...
                                                     std::string energy_str = (has_rapl && energy_j >= 0) ? fmt(energy_j, 3) : "";
                                                     std::string power_str = (has_rapl && avg_power_w >= 0) ? fmt(avg_power_w, 1) : "";
                                                                  ...
                                                                  size_t calibrateWithWarmup(real* a, const real* b, const real* c,
                                                                      ...
                                                                      return passes;
                                                                                             ...
                                                                                             int main() {
                                                                                                 ...
                                                                                                 // AMD-robust: Report socket count and total LLC (summed across all L3 slices)
                                                                                                 int num_sockets = getNumSockets();
                                                                                                 size_t llc_total = getTotalCPULLCSize();
                                                                                                 ...
                                                                                                 return EXIT_FAILURE;
                                                                                                     ...
                                                                                                     for (const auto& size_info : size_list) {
                                                                                                         ...
                                                                                                         for (int run = 0; run < REPEATS; run++) {
                                                                                                             ...
                                                                                                             // Handle missing/bad energy properly
                                                                                                             double energy_j = -1.0;
