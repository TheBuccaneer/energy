#!/usr/bin/env python3
"""
Day-Ahead Price Enricher for STREAM measurements (Sept 2024 format).
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="Enrich STREAM measurement CSVs with Day-Ahead prices (Sept 2024)"
    )
...
def enrich_measurement_file(file_path, price_df, price_sync_dict, inplace, issues_log, qa_log):
    ...
    STREAM-specific: Uses passes_e2e (not batches) for per-pass calculations
    """
    try:
        ...
        # === STEP 5: Pass metrics (STREAM uses passes_e2e, not batches!) ===
        # In STREAM: 'batches' is just a counter/ID, 'passes_e2e' is the actual repetition count
        if 'passes_e2e' in df.columns:
            df['passes_e2e'] = pd.to_numeric(df['passes_e2e'], errors='coerce')
            
            # Calculate kWh per pass if not already present
            if 'kWh_per_pass' not in df.columns:
                df['kWh_per_pass'] = df['energy_j'] / df['passes_e2e'] / 3.6e6
            else:
                if df['kWh_per_pass'].dtype == 'object':
                    df['kWh_per_pass'] = df['kWh_per_pass'].astype(str).str.replace(',', '.', regex=False)
                df['kWh_per_pass'] = pd.to_numeric(df['kWh_per_pass'], errors='coerce')
            
            # Calculate pass costs for all areas
            for area in ['DE', 'FR', 'PL']:
                area_lower = area.lower()
                df[f'{area_lower}_price_eur_kwh_pass'] = df[f'{area_lower}_price_eur_kwh_run']
                df[f'{area_lower}_eur_pass'] = df['kWh_per_pass'] * df[f'{area_lower}_price_eur_kwh_pass']
                df[f'{area_lower}_eur_pass_sync'] = df['kWh_per_pass'] * price_sync_dict[area]
                df[f'{area_lower}_eur_job_from_passes'] = df[f'{area_lower}_eur_pass'] * df['passes_e2e']
            
            # Backward-compatible pass columns
            df['price_eur_kwh_pass'] = df.apply(
                lambda row: row[f'{row["market_area"].lower()}_price_eur_kwh_pass'],
                axis=1
            )
            df['eur_pass'] = df.apply(
                lambda row: row[f'{row["market_area"].lower()}_eur_pass'],
                axis=1
            )
            df['eur_pass_sync'] = df.apply(
                lambda row: row[f'{row["market_area"].lower()}_eur_pass_sync'],
                axis=1
            )
            df['eur_job_from_passes'] = df.apply(
                lambda row: row[f'{row["market_area"].lower()}_eur_job_from_passes'],
                axis=1
            )
            ...
            mismatch = df[
                (df['eur_job'].notna()) & 
                (df['eur_job_from_passes'].notna()) &
                (np.abs((df['eur_job'] - df['eur_job_from_passes']) / df['eur_job']) > 0.01)
            ]
            if not mismatch.empty:
                qa_log.write(f"{file_path.name}: {len(mismatch)} rows with >1% job/pass mismatch\n")
