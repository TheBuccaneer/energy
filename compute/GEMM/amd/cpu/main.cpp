// main.cpp - CPU GEMM Energy Benchmark with OpenBLAS (Intel & AMD)
//
...
// - RAPL access for energy measurements (REQUIRED - no fallback)
...
constexpr int    NUM_THREADS      = 64;   // OpenBLAS threads
...
double readRAPLEnergy() {
    // Read sum of all package-zone energies from RAPL (Intel & AMD)
    // Returns energy in Joules, or -1.0 if not accessible

    std::vector<std::string> candidates;

    // Collect all RAPL zones (intel-rapl & amd-rapl)
...
void writeCSVRow(std::ofstream& file, const std::string& host,
    ...
    file << "NA" << "\n";  // throttle_reasons
                 ...
                 int main() {
                     ...
                     // Test RAPL availability - REQUIRED (no fallback)
                     double rapl_test = readRAPLEnergy();

                     if (rapl_test < 0) {
                         std::cerr << "ERROR: RAPL energy measurement not accessible.\n"
                         << "This benchmark requires RAPL/powercap access for energy measurements.\n"
                         << "\n"
                         << "Possible causes:\n"
                         << "  - Insufficient permissions (run as root or add udev rules)\n"
                         << "  - RAPL not available on this system\n"
                         << "  - MSR module not loaded (try: sudo modprobe msr)\n"
                         << "\n"
                         << "See documentation for RAPL setup instructions.\n";
                         return EXIT_FAILURE;
                     }
