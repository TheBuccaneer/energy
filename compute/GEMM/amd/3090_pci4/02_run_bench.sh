...
# Script to compile and run gemm_bench 10 times with cooling periods
# Saves data folders sequentially: data_1/, data_2/, ..., data_10/

set -e  # Exit on error

echo "========================================"
echo "CUDA GEMM Benchmark - Automated Runner"
echo "========================================"
echo ""
...
echo "Step 1: Compiling gemm_bench..."
echo "Command: nvcc -O3 -std=c++17 -o gemm_bench main.cu -lcublas -lnvidia-ml"
echo ""

nvcc -O3 -std=c++17 -o gemm_bench main.cu -lcublas -lnvidia-ml
...
if [ ! -f "gemm_bench" ]; then
    echo "ERROR: gemm_bench binary not found after compilation!"
    exit 1
fi
...
cat data_*/raw/energy_benchmark_rtx3090.csv | awk 'NR==1 || !/^timestamp/' > merged_all_runs.csv
echo ""
echo "To merge all runs into one CSV:"
echo "  cat data_*/raw/energy_benchmark_rtx3090.csv | awk 'NR==1 || !/^timestamp/' > merged_all_runs.csv"
echo ""
echo "Done!"
