...
def check_source_provenance(checks: list[dict], platform_root: Path, platform: str, out: Path) -> None:
    source = locate_source(platform_root)
    runner = locate_runner(platform_root, platform)
    if source is None:
        add_check(checks, 'provenance', 'cuda_source_present', 'FAIL', False,
                  'not found', 'scripts/STRIDED_GEMM/main_strided_gemm.cu')
    else:
        ...
        tokens = {
            ...
            'implementation_token':'cublas_gemm_ex_fp32_pedantic_ld2n',
            'padding_sentinel':'PADDING_C',
            'size_filter':'BENCH_SIZE_FILTER',
            'gpu_name_filter':'BENCH_EXPECTED_GPU',
        }
        ...
        regexes = {
            ...
            'init_divides_by_ld': r'row\s*=.*index\s*/.*ld',
            'init_modulo_ld': r'col\s*=.*index\s*%.*ld',
            'all_leading_dimensions_use_ld': r'b\s*,\s*CUDA_R_32F\s*,\s*ld\s*,.*a\s*,\s*CUDA_R_32F\s*,\s*ld\s*,.*c\s*,\s*CUDA_R_32F\s*,\s*ld',
            'problem_spec_records_ld': r'N=.*;ld=',
        }
        ...
        forbidden = [
            'cublasSgemmStridedBatched', 'batch_count', 'SIZE_TO_BATCH_COUNTS',
            'MACRO_REPEATS', 'gpu_e2e', 'sleep_for', 'Cooling down',
        ]
        found = [token for token in forbidden if token in text]
        add_check(checks, 'provenance', 'forbidden_legacy_tokens', 'FAIL', not found,
                  found or 'none', 'none')
        (out/'audited_cuda_source_path.txt').write_text(str(source)+'\n', encoding='utf-8')

    if runner is None:
        add_check(checks, 'provenance', 'runner_present', 'FAIL', False,
                  'not found', '02_run_GPU_<GPU>_STRIDED_GEMM_only.sh')
    else:
        ...
        pause_tokens = [token for token in ['SESSION_PAUSE','sleep 300','Cooling for'] if token in text]
        add_check(checks, 'provenance', 'no_session_pause', 'FAIL', not pause_tokens,
                  pause_tokens or 'none', 'none')
        (out/'audited_runner_path.txt').write_text(str(runner)+'\n', encoding='utf-8')


def main() -> None:
    parser = argparse.ArgumentParser(description='Validate one GPU STRIDED_GEMM campaign')
    parser.add_argument('--campaign')
    args = parser.parse_args()
    _, platform, platform_root, run_dir, out, cfg = context(__file__)
    ...
    expected_sets = {
        'schema_version':{'cpu-gpu-v2'}, 'workload':{'STRIDED_GEMM'},
        'implementation':{'cublas_gemm_ex_fp32_pedantic_ld2n'}, 'execution_mode':{'gpu_resident'},
    }
    ...
    add_check(checks, 'semantics', 'device_name', 'FAIL',
              len(names)==1 and cfg['device_token'] in names[0], names, f"one {cfg['label']}")
    ...
    add_check(checks, 'semantics', 'problem_spec', 'FAIL',
              bool((raw.problem_spec.astype(str)==expected_spec).all()), 'checked', 'N=<size>;ld=<2size>')
    ...
    formulas = {
        ...
        'logical_bytes_excludes_padding':rel_close(raw.logical_bytes_per_op, expected_bytes, 2e-9, 1e-3),
    ...
    for session, group in raw.groupby('session_number'):
        if sorted(group.sequence_index.astype(int)) != list(range(1,expected_rows+1)):
            seq_bad += 1
        ids = set(group.session_id.astype(str))
        if not (len(ids)==1 and all(campaign.stamp in sid and sid.endswith(f'_session{session}') for sid in ids)):
            sid_bad += 1
    ...
    check_source_provenance(checks, platform_root, platform, out)
    ...
    pd.DataFrame([{
        'platform':platform, 'campaign':campaign.stamp, 'files':len(campaign.files), 'rows':len(raw),
        'run_directory':str(run_dir), 'device_name':'; '.join(names),
    ...
    report = (
        f"# {cfg['label']} STRIDED_GEMM validation report\n\n"
        f'- Campaign: `{campaign.stamp}`\n- Files: {len(campaign.files)}\n- Rows: {len(raw)}\n'
        f'- Expected rows/session: {expected_rows}\n- Overall verdict: **{verdict}**\n\n'
        f'## Failed checks\n\n{markdown_table(hard)}\n\n'
        f'## Warnings\n\n{markdown_table(warns)}\n\n'
        f'## All checks\n\n{markdown_table(check_df, 250)}\n\n'
        '## Metric semantics\n\n'
        'This campaign measures resident FP32 STRIDED_GEMM with logical N×N matrices and `ld=2N`. '
        'Allocation, initialization, checksum copies and PCIe transfers are outside the measured interval. '
        'Logical bytes remain 12N² as a semantic anchor, while the allocated A+B+C footprint is 24N². '
        'Neither value is a direct measurement of physical device-memory traffic. NVML energy is board-level.\n'
    )
    (out/'validation_report.md').write_text(report, encoding='utf-8')
    print(f"[{cfg['label']}] STRIDED_GEMM validation: {verdict}")
