...
from stream_analysis_common import *
...
def main() -> None:
    args = parse_args("Analyze one validated STREAM campaign")
    ...
    if not checks_path.is_file() or not manifest_path.is_file():
        raise SystemExit("Run 01_validate_stream.py first.")
    ...
    for size in SIZES:
        ...
        trade_rows.append({
            ...
            "problem_size": size,
            "working_set_gib": 12.0 * size / (1024 ** 3),
    ...
    plot_metric(summary, "runtime_per_op_s", "Laufzeit pro Triad [s]", ctx.figure_dir / "runtime_by_size.png", logy=True)
    plot_metric(summary, "primary_energy_per_op_j", "Primärenergie pro Triad [J]", ctx.figure_dir / "energy_by_size.png", logy=True)
    ...
    report = f"""# STREAM scientific report — {ctx.config['label']}
...
"""
    ...
    print(f"[{ctx.platform} STREAM] analysis complete: {ctx.result_dir}")
