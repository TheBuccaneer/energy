...
cleanup() {
    ...
    if [[ "$SUCCESS" -eq 1 && "$POWER_OFF_AT_END" -eq 1 ]]; then
        echo "All RTX 3090 STRIDED_GEMM sessions completed successfully. Powering off."
        sudo systemctl poweroff
    elif [[ "$SUCCESS" -eq 1 ]]; then
        echo "All RTX 3090 STRIDED_GEMM sessions completed successfully. POWER_OFF_AT_END=0."
    else
        echo "RTX 3090 STRIDED_GEMM run stopped or failed. GPU restored; no shutdown." >&2
    fi
    exit "$status"
}
trap cleanup EXIT
...
for ((session=1; session<=SESSIONS; session++)); do
    ...
    output="$OUTDIR/strided_gemm_3090_${session_id}.csv"

    echo "=== RTX 3090 STRIDED_GEMM | session $session/$SESSIONS | $session_id | reps=$REPS ==="
    ...
    echo "RTX 3090 STRIDED_GEMM session $session/$SESSIONS complete: $rows rows."
done

echo "Completed $SESSIONS RTX 3090 STRIDED_GEMM sessions with no pause between sessions."
