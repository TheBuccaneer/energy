...
__global__ void initialize_matrices(float* a, float* b, float* c, int n) {
...
Options parse_options(int argc, char** argv) {
    Options options;
    options.output_file = argc > 1 ? argv[1] : "gemm_gpu.csv";
...
void gemm(cublasHandle_t handle, const float* a, const float* b, float* c, int n) {
...
double measure_kernel_seconds(cublasHandle_t handle, const float* a, const float* b,
                              float* c, int n, int batches, cudaStream_t stream,
                              cudaEvent_t start, cudaEvent_t stop) {
    CUDA_CHECK(cudaEventRecord(start, stream));
    for (int batch = 0; batch < batches; ++batch) gemm(handle, a, b, c, n);
...
int calibrate(cublasHandle_t handle, const float* a, const float* b, float* c,
              int n, cudaStream_t stream, cudaEvent_t start, cudaEvent_t stop) {
    gemm(handle, a, b, c, n);
    ...
    for (int step = 0; step < MAX_CALIBRATION_STEPS; ++step) {
        const double seconds =
            measure_kernel_seconds(handle, a, b, c, n, batches, stream, start, stop);
...
bool correct(const float* device_c, int n) {
    ...
    for (const auto& [row, col] : samples) {
        ...
        CUDA_CHECK(cudaMemcpy(&actual, device_c + index, sizeof(float), cudaMemcpyDeviceToHost));
...
void write_row(std::ofstream& output, const ResultRow& row) {
    output << SCHEMA_VERSION << ',' << timestamp() << ',' << csv_escape(row.session_id) << ','
           ...
           << csv_escape("GEMM") << ',' << csv_escape("cublas_gemm_ex_fp32_pedantic") << ','
           << "gpu_resident" << ',' << csv_escape(row.device_name) << ',' << -1 << ','
           << row.problem_size << ',' << csv_escape("N=" + std::to_string(row.problem_size)) << ','
           << row.batches << ','
...
void print_result(const ResultRow& row) {
    std::cout << "[GEMM] N=" << row.problem_size
              << " rep=" << row.repetition
              << " batches=" << row.batches
    ...
    try {
        ...
        for (int n : sizes) {
            ...
            if (count > std::numeric_limits<size_t>::max() / sizeof(float)) {
                throw std::runtime_error("Matrix size overflow for N=" + std::to_string(n));
            }
            ...
            if (bytes > free_bytes / 3) {
                throw std::runtime_error(
                    "Insufficient free GPU memory for three N=" + std::to_string(n) +
                    " matrices: need " + std::to_string(3.0 * bytes / (1024.0 * 1024.0 * 1024.0)) +
                    " GiB, free " + std::to_string(free_bytes / (1024.0 * 1024.0 * 1024.0)) + " GiB");
            }
            ...
            initialize_matrices<<<blocks, threads, 0, stream>>>(a, b, c, n);
            CUDA_CHECK(cudaGetLastError());
            CUDA_CHECK(cudaStreamSynchronize(stream));

            const int batches = calibrate(handle, a, b, c, n, stream, start_event, stop_event);
