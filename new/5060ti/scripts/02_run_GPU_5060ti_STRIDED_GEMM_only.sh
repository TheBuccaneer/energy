...
OUTDIR="$ROOT/runs/STRIDED_GEMM"
RESTORE="$ROOT/scripts/03_disable_GPU_5060ti.sh"
...
cleanup() {
    ...
    if [[ "$SUCCESS" -eq 1 && "$POWER_OFF_AT_END" -eq 1 ]]; then
        echo "All RTX 5060 Ti STRIDED_GEMM sessions completed successfully. Powering off."
        sudo systemctl poweroff
    elif [[ "$SUCCESS" -eq 1 ]]; then
        echo "All RTX 5060 Ti STRIDED_GEMM sessions completed successfully. POWER_OFF_AT_END=0."
    else
        echo "RTX 5060 Ti STRIDED_GEMM run stopped or failed. GPU restored; no shutdown." >&2
    fi
    exit "$status"
}
trap cleanup EXIT
...
for ((session=1; session<=SESSIONS; session++)); do
    ...
    seed=$((20264000 + session))
    output="$OUTDIR/strided_gemm_5060ti_${session_id}.csv"

    echo "=== RTX 5060 Ti STRIDED_GEMM | session $session/$SESSIONS | $session_id | reps=$REPS ==="
    ...
    echo "RTX 5060 Ti STRIDED_GEMM session $session/$SESSIONS complete: $rows rows."
done

echo "Completed $SESSIONS RTX 5060 Ti STRIDED_GEMM sessions with no pause between sessions."
