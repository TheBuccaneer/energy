...
SRC="$SCRIPT_DIR/REDUCTION/main_reduction.cu"
BIN="$SCRIPT_DIR/REDUCTION/main_reduction"
OUTDIR="$PLATFORM_DIR/runs/REDUCTION"
...
for candidate in 03_disable_GPU_5060Ti.sh 03_disable_GPU_5060ti.sh 03_disable_GPU_5060TI.sh; do
    [[ -f "$SCRIPT_DIR/$candidate" ]] && { RESTORE="$SCRIPT_DIR/$candidate"; break; }
done
