...
METRICS = ['runtime_per_op_s','device_energy_per_op_j','gflops_per_s','avg_power_w','temp_c','sm_clock_mhz','edp_j_s']
...
def build_summary(session: pd.DataFrame, raw: pd.DataFrame) -> pd.DataFrame:
    ...
    for n, group in session.groupby('problem_size'):
        ...
        row['stable_throughput'] = row['gflops_per_s_session_cv_pct'] <= 5.0
        rows.append(row)
    return pd.DataFrame(rows).sort_values('problem_size')


def plot_metric(summary, metric, ylabel, filename, figdir, logy=True):
...
def main() -> None:
    ...
    for _, group in df.groupby(['session_number','problem_size']):
        mask = robust_outlier_mask(group, ['runtime_per_op_s','device_energy_per_op_j','gflops_per_s','avg_power_w'])
        if mask.any():
    ...
    session = df.groupby(['session_number','problem_size'], as_index=False)[
        METRICS+['kernel_fraction','clock_change_pct','temperature_rise_c']
    ].median()
    ...
    overview = df.groupby('session_number', as_index=False).agg(
        rows=('problem_size','size'), median_gflops=('gflops_per_s','median'),
    ...
    figdir = out/'figures'
    plot_metric(summary, 'gflops_per_s', 'Throughput (GFLOP/s)', 'throughput_by_size.png', figdir, False)
    plot_metric(summary, 'device_energy_per_op_j', 'Board energy per GEMM (J)', 'energy_per_gemm.png', figdir)
    plot_metric(summary, 'runtime_per_op_s', 'Runtime per GEMM (s)', 'runtime_per_gemm.png', figdir)
    plot_metric(summary, 'edp_j_s', 'Energy-delay product (J·s)', 'edp_by_size.png', figdir)
    ...
    pivot = session.pivot(index='problem_size', columns='session_number', values='gflops_per_s')
    plt.figure(figsize=(8,5))
    ...
    peak = summary.loc[summary.gflops_per_s_median.idxmax()]
    best_edp = summary.loc[summary.edp_j_s_median.idxmin()]
    ...
    report = (
        ...
        '## Main findings\n\n'
        f'- Peak median throughput: {peak.gflops_per_s_median:.2f} GFLOP/s at N={int(peak.problem_size)}.\n'
        f'- Lowest median EDP: {best_edp.edp_j_s_median:.6g} J·s at N={int(best_edp.problem_size)}.\n\n'
        '## Interpretation\n\n'
        'This analysis characterizes resident FP32 GEMM. PCIe transfers are outside the measured interval. '
        'NVML device energy is board-level and includes device-memory energy, so it is not the same measurement domain as CPU package-only RAPL. '
