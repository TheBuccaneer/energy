...
def validate_campaign(ctx: Context, campaign: Campaign) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if cfg["kind"] == "CPU":
        ...
        # CUDA-event duration and host steady-clock wall duration are measured
        # on independent clock domains. Therefore kernel_time_s <= e2e_time_s
        # is not a strict row-wise identity, even though the wall interval
        # semantically encloses the GPU work. Small crossings are diagnostic,
        # not measurement failures.
        kernel_time = data.kernel_time_s.to_numpy(float)
        e2e_time = data.e2e_time_s.to_numpy(float)
        kernel_minus_e2e = kernel_time - e2e_time

        strict_rounding_tolerance_s = 2e-6
        slight_crossing = kernel_minus_e2e > strict_rounding_tolerance_s

        # A hard failure is reserved for a material disagreement: more than
        # both 0.5 ms and 0.5% of the host wall interval.
        material_tolerance_s = np.maximum(5e-4, 5e-3 * e2e_time)
        material_crossing = kernel_minus_e2e > material_tolerance_s

        max_positive_delta_s = float(np.maximum(kernel_minus_e2e, 0.0).max())
        max_ratio = float(np.max(kernel_time / e2e_time))

        add_check(
            checks,
            "timing",
            "gpu_kernel_not_materially_above_e2e",
            "FAIL",
            bool((~material_crossing).all()),
            {
                "rows": int(material_crossing.sum()),
                "max_kernel_minus_e2e_ms": max_positive_delta_s * 1000.0,
                "max_kernel_over_e2e": max_ratio,
            },
            {
                "rows": 0,
                "tolerance": "max(0.5 ms, 0.5% of e2e_time_s)",
            },
        )
        add_check(
            checks,
            "timing",
            "gpu_kernel_slightly_above_e2e",
            "WARN",
            not bool(slight_crossing.any()),
            {
                "rows": int(slight_crossing.sum()),
                "max_kernel_minus_e2e_ms": max_positive_delta_s * 1000.0,
                "max_kernel_over_e2e": max_ratio,
            },
            "small cross-clock differences are allowed; report them",
        )
