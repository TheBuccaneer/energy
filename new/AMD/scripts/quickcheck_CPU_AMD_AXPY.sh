...
QUICK_THREADS="1,64"
QUICK_REPS=2
QUICK_SEED=$((0x4158505900001900))

finish_action() {
    local status=$1
    if (( SUCCESS == 1 && status == 0 )); then
        echo "[done] AMD AXPY quickcheck completed. No official session was started."
    else
        echo "[abort] AMD AXPY quickcheck failed." >&2
    fi
}
...
lines = [x for x in text.splitlines() if x.startswith("[ANTI_COLLAPSE] ")]
if len(lines) != 1:
    raise SystemExit(f"expected one anti-collapse line, got {len(lines)}")
fields = {}
for token in lines[0].split()[1:]:
    if "=" in token:
        k,v = token.split("=",1)
        fields[k] = v
required = {"N","threads","B_probe","two_B_probe","t1","t2","ratio",
            "time_basis","checksum1","checksum2","gate"}
missing = required - fields.keys()
if missing:
    raise SystemExit(f"anti-collapse missing fields: {sorted(missing)}")
if int(fields["N"]) != 1000000 or int(fields["threads"]) != 64:
    raise SystemExit("anti-collapse wrong AMD configuration")
b = int(fields["B_probe"])
two_b = int(fields["two_B_probe"])
t1 = float(fields["t1"])
t2 = float(fields["t2"])
ratio = float(fields["ratio"])
if b < 100 or two_b != 2*b or two_b > 250000:
    raise SystemExit("anti-collapse invalid batch counts")
if t1 < .020 or t2 < .020:
    raise SystemExit("anti-collapse timing too short")
if not all(math.isfinite(x) for x in (t1,t2,ratio)):
    raise SystemExit("anti-collapse non-finite value")
if not 1.7 <= ratio <= 2.3:
    raise SystemExit(f"anti-collapse ratio out of range: {ratio}")
if fields["time_basis"] != "wall_time_s":
    raise SystemExit("anti-collapse wrong time basis")
if fields["checksum1"] != "OK" or fields["checksum2"] != "OK" or fields["gate"] != "PASS":
    raise SystemExit("anti-collapse gate failed")
print(f"validated anti-collapse gate: ratio={ratio:.6f}")
PYLOG
}

stamp=$(date +%Y%m%d_%H%M%S)
session_id="axpy_amd_${stamp}_quickcheck"
...
run_axpy "$csv" "$log" "$QUICK_REPS" "$session_id" "$QUICK_SEED" \
    "$QUICK_SIZES" "$QUICK_THREADS" 1

validate_csv "$csv" "$QUICK_SIZES" "$QUICK_THREADS" "$QUICK_REPS" "$session_id"
validate_probe_log "$log"

echo "[quickcheck] PASS: $csv"
SUCCESS=1
