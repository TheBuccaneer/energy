...
def project_root(script_file: str | Path) -> Path:
    ...
    # .../new/ALL AUDIT/GEMM/analyse/file.py -> .../new
    return Path(script_file).resolve().parents[3]


def audit_root(script_file: str | Path) -> Path:
    return project_root(script_file) / "ALL AUDIT" / "GEMM"
...
def find_complete_campaigns(run_dir: Path, platform: str) -> dict[str, list[Path]]:
    patterns = {
        "INTEL": re.compile(r"^gemm_intel_(\d{8}_\d{6})_session([1-5])\.csv$", re.I),
        "AMD": re.compile(r"^gemm_amd_(\d{8}_\d{6})_session([1-5])\.csv$", re.I),
        "3090": re.compile(r"^gemm_3090_(\d{8}_\d{6})_session([1-5])\.csv$", re.I),
        "5060ti": re.compile(r"^gemm_5060ti_(\d{8}_\d{6})_session([1-5])\.csv$", re.I),
    }
