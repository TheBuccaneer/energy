...
def results_dir(script_file: str | Path) -> Path:
    out = project_root(script_file) / "ALL AUDIT" / "STREAM" / "results"
    (out / "figures").mkdir(parents=True, exist_ok=True)
    return out


def platform_result_dir(script_file: str | Path, platform: str) -> Path:
    return project_root(script_file) / platform / "results" / "STREAM"


def platform_run_dir(script_file: str | Path, platform: str) -> Path:
    return project_root(script_file) / platform / "runs" / "STREAM"
