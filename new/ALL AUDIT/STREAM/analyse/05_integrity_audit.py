...
def main() -> None:
    ...
    bw_expected = 12.0 * n / sessions.runtime_s.to_numpy(float) / 1e9
    gbj_expected = 12.0 * n / sessions.energy_j.to_numpy(float) / 1e9
    edp_expected = sessions.runtime_s.to_numpy(float) * sessions.energy_j.to_numpy(float)
    throughput_expected = 2.0 * n / sessions.runtime_s.to_numpy(float) / 1e9
    efficiency_expected = 2.0 * n / sessions.energy_j.to_numpy(float) / 1e9
    identities = {
    ...
    record(checks, "semantics", "logical_bandwidth_scope", "WARN", False,
           "12*N/runtime", "not measured physical traffic")
    ...
    report = f"""# Independent integrity audit — STREAM
...
"""
    ...
    print(f"[ALL STREAM] integrity audit: {verdict}")
    if len(hard):
        raise SystemExit(2)
