...
def main() -> None:
    ...
    # Descriptive exact-winner regret. Selection and intervals use the same five sessions.
    regret_rows = []
    for platform in PLATFORMS:
        for size in SIZES:
            e = leaders[(leaders.platform == platform) & (leaders.problem_size == size) & (leaders.policy == "energy_opt")].iloc[0]
            r = leaders[(leaders.platform == platform) & (leaders.problem_size == size) & (leaders.policy == "runtime_opt")].iloc[0]
            r_cfg = str(r.exact_configuration)
            e_cfg = str(e.exact_configuration)
            r_sessions = sessions[(sessions.platform == platform) & (sessions.problem_size == size) & (sessions.configuration == r_cfg)]
            e_sessions = sessions[(sessions.platform == platform) & (sessions.problem_size == size) & (sessions.configuration == e_cfg)]
            r_energy = r_sessions.energy_j.to_numpy(float)
            e_energy = e_sessions.energy_j.to_numpy(float)
            r_runtime = r_sessions.runtime_s.to_numpy(float)
            e_runtime = e_sessions.runtime_s.to_numpy(float)
            energy_ratio = float(np.median(r_energy) / np.median(e_energy))
            energy_lo, energy_hi = bootstrap_ratio_ci(r_energy, e_energy, ("exact_winner_regret", "energy", platform, size, r_cfg, e_cfg))
            runtime_ratio = float(np.median(r_runtime) / np.median(e_runtime))
            runtime_lo, runtime_hi = bootstrap_ratio_ci(r_runtime, e_runtime, ("exact_winner_regret", "runtime", platform, size, r_cfg, e_cfg))
            regret_rows.append({
                "platform": platform, "platform_label": LABELS[platform], "problem_size": size,
                "runtime_opt_configuration": r_cfg, "energy_opt_configuration": e_cfg,
                "same_configuration": r_cfg == e_cfg,
                "runtime_opt_over_energy_opt_energy_ratio": energy_ratio,
                "energy_ratio_ci95_low": energy_lo, "energy_ratio_ci95_high": energy_hi,
                "runtime_opt_energy_penalty_pct": 100.0 * (energy_ratio - 1.0),
                "energy_penalty_classification": classify_ratio(energy_ratio, energy_lo, energy_hi, "runtime_opt", "energy_opt", True),
                "runtime_opt_over_energy_opt_runtime_ratio": runtime_ratio,
                "runtime_ratio_ci95_low": runtime_lo, "runtime_ratio_ci95_high": runtime_hi,
                "runtime_opt_runtime_gain_pct": 100.0 * (1.0 - runtime_ratio),
                "runtime_gain_classification": classify_ratio(runtime_ratio, runtime_lo, runtime_hi, "runtime_opt", "energy_opt", True),
                "analysis_type": "descriptive_exact_winner_post_selection_independent_sessions",
            })
    pd.DataFrame(regret_rows).to_csv(out / "within_platform_exact_winner_regret.csv", index=False)

    print(f"[ALL REDUCTION] unified statistics: {out}")
