...
def main() -> None:
    ...
    placement_rows = []
    for size in SIZES:
        row = {"problem_size": size, "working_set_gib": (4.0 * size + 4.0) / (1024 ** 3)}
        for metric, policy in METRIC_POLICY.items():
    ...
    figdir = out / "figures"
    plot_native(leaders, "runtime_s", "runtime_opt", "Laufzeit pro Reduktion [s]", figdir / "all_platform_runtime_native_best.png", True)
    plot_native(leaders, "energy_j", "energy_opt", "Primärenergie pro Reduktion [J]", figdir / "all_platform_energy_native_best.png", True)
    ...
    print(f"[ALL REDUCTION] comparisons: {out}")
