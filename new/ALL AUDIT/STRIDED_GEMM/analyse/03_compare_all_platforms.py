...
def plot_native_metric(leaders: pd.DataFrame, metric: str, policy: str, ylabel: str, filename: str, figdir, logy: bool = True) -> None:
    ...
    plt.ylabel(ylabel)
    plt.title(f"STRIDED_GEMM native-best comparison: {ylabel}")
...
def plot_pareto_by_size(pareto: pd.DataFrame, figdir) -> None:
    for size in SIZES:
        ...
        plt.xscale("log")
        plt.yscale("log")
        plt.xlabel("Runtime per STRIDED_GEMM (s)")
        plt.ylabel("Measured device-domain energy per STRIDED_GEMM (J)")
        plt.title(f"STRIDED_GEMM energy-runtime configurations at N={size}")
        plt.grid(True, which="both", alpha=0.3)
        plt.legend()
        plt.tight_layout()
...
def main() -> None:
    ...
    figdir = out / "figures"
    plot_native_metric(leaders, "runtime_s", "runtime_opt", "Runtime per STRIDED_GEMM (s)", "all_platform_runtime_native_best.png", figdir)
    plot_native_metric(leaders, "energy_j", "energy_opt", "Measured device-domain energy per STRIDED_GEMM (J)", "all_platform_energy_native_best.png", figdir)
    ...
    print(f"[ALL STRIDED_GEMM] all-platform comparisons written to {out}")
