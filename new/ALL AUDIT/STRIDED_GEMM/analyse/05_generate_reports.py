    ...
    for metric, group in winners.groupby("metric"):
        for platform, count in group.exact_winner.value_counts().items():
            ...
            rows.append({
                ...
                "platform_label": PLATFORM_LABELS[str(platform)],
...
def main() -> None:
    ...
    for frame in [manifest, summary, leaders, pairwise, winners, best_cpu_gpu, placement, within, pareto, tradeoffs, stability]:
    ...
    audit = f"""# Combined audit of all STRIDED_GEMM campaigns
...
comparisons.
"""
    (out / "ALL_STRIDED_GEMM_AUDIT_REPORT.md").write_text(audit, encoding="utf-8")

    results_summary = f"""# All-platform STRIDED_GEMM results summary

We compared FP32 STRIDED_GEMM across two CPUs and two GPUs using {total_rows:,} validated
...
"""
    (out / "ALL_STRIDED_GEMM_RESULTS_SUMMARY.md").write_text(results_summary, encoding="utf-8")

    handoff = f"""# Handoff for Claude: combined Intel, AMD, RTX 3090, and RTX 5060 Ti STRIDED_GEMM analysis
...
"""
    (out / "ALL_STRIDED_GEMM_HANDOFF_FOR_CLAUDE.md").write_text(handoff, encoding="utf-8")

    methods = """# Combined STRIDED_GEMM methods and limitations
...
"""
    ...
    print(f"[ALL STRIDED_GEMM] reports written to {out}")
