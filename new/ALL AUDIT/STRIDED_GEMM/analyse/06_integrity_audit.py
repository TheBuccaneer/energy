...
def main() -> None:
    ...
    for frame in [session, summary, leaders, selected, pairwise, winners, best_cpu_gpu, placement, pareto, tradeoff, stability, preflight]:
        for column in ["platform", "exact_winner", "platform_a", "platform_b", "best_cpu", "best_gpu"]:
    ...
    breakdown = all_platforms.merge(breakdown, on="platform", how="left")
    for col in ["unstable_configurations", "runtime_unstable", "energy_unstable", "throughput_unstable"]:
        breakdown[col] = pd.to_numeric(breakdown[col], errors="coerce").fillna(0).astype(int)
    ...
    # Independent audit of the optional dense-versus-strided outputs.
    matched_path = out / "dense_vs_strided_configuration_matched.csv"
    native_path = out / "dense_vs_strided_native_best.csv"
    layout_placement_path = out / "layout_induced_placement_changes.csv"
    if matched_path.is_file() and native_path.is_file():
        matched = pd.read_csv(matched_path)
        native = pd.read_csv(native_path)
        if not matched.empty:
            record(checks, "dense_vs_strided", "configuration_matched_row_count", "FAIL",
                   len(matched) == 648, len(matched), 648)
            ratio_error = np.abs(
                matched.strided_over_dense_ratio
                - matched.strided_median / matched.dense_median
            )
            record(checks, "dense_vs_strided", "matched_ratio_identity", "FAIL",
                   float(ratio_error.max()) < 1e-12,
                   f"max_abs_error={float(ratio_error.max()):.3e}", "<1e-12")
            low = 1.0 / (1.0 + PRACTICAL_TOLERANCE)
            high = 1.0 + PRACTICAL_TOLERANCE
            expected = []
            for row in matched.itertuples():
                if row.ci95_high < low:
                    expected.append("clear_strided_lower")
                elif row.ci95_low > high:
                    expected.append("clear_strided_higher")
                elif low <= row.strided_over_dense_ratio <= high:
                    expected.append("practically_equivalent_or_uncertain")
                else:
                    expected.append("uncertain_strided_lower" if row.strided_over_dense_ratio < 1 else "uncertain_strided_higher")
            class_bad = int((matched.classification.astype(str).to_numpy() != np.asarray(expected)).sum())
            record(checks, "dense_vs_strided", "matched_classification_rule", "FAIL",
                   class_bad == 0, class_bad, 0)
        else:
            record(checks, "dense_vs_strided", "configuration_matched_available", "WARN",
                   False, "empty", "dense GEMM unified results available")
        if not native.empty:
            record(checks, "dense_vs_strided", "native_best_row_count", "FAIL",
                   len(native) == 144, len(native), 144)
            record(checks, "dense_vs_strided", "native_best_post_selection_label", "FAIL",
                   set(native.analysis_type.astype(str)) == {"descriptive_native_best_post_selection_independent_sessions"},
                   sorted(set(native.analysis_type.astype(str))),
                   "descriptive_native_best_post_selection_independent_sessions")
        else:
            record(checks, "dense_vs_strided", "native_best_available", "WARN",
                   False, "empty", "dense GEMM unified results available")

        if layout_placement_path.is_file():
            layout_placement = pd.read_csv(layout_placement_path)
            if not layout_placement.empty:
                required_columns = {
                    "point_estimate_winner_changed",
                    "leader_set_changed",
                    "decisive_placement_changed",
                }
                missing_columns = sorted(required_columns - set(layout_placement.columns))
                record(checks, "dense_vs_strided", "placement_change_columns_present", "FAIL",
                       not missing_columns, missing_columns, [])
                record(checks, "dense_vs_strided", "placement_change_row_count", "FAIL",
                       len(layout_placement) == 27, len(layout_placement), 27)

                if not missing_columns:
                    expected_point = (
                        layout_placement.dense_exact_winner.astype(str)
                        != layout_placement.strided_exact_winner.astype(str)
                    )
                    expected_set = pd.Series(
                        [
                            parse_leader_set(d) != parse_leader_set(s)
                            for d, s in zip(
                                layout_placement.dense_leader_platforms,
                                layout_placement.strided_leader_platforms,
                            )
                        ],
                        index=layout_placement.index,
                    )
                    expected_decisive = (
                        layout_placement.dense_selection_status.astype(str).eq("clear_leader")
                        & layout_placement.strided_selection_status.astype(str).eq("clear_leader")
                        & expected_point
                    )
                    observed_point = layout_placement.point_estimate_winner_changed.map(parse_bool)
                    observed_set = layout_placement.leader_set_changed.map(parse_bool)
                    observed_decisive = layout_placement.decisive_placement_changed.map(parse_bool)
                    point_bad = int((observed_point != expected_point).sum())
                    set_bad = int((observed_set != expected_set).sum())
                    decisive_bad = int((observed_decisive != expected_decisive).sum())
                    record(checks, "dense_vs_strided", "point_estimate_change_rule", "FAIL",
                           point_bad == 0, point_bad, 0)
                    record(checks, "dense_vs_strided", "leader_set_order_invariant_rule", "FAIL",
                           set_bad == 0, set_bad, 0)
                    record(checks, "dense_vs_strided", "decisive_placement_change_rule", "FAIL",
                           decisive_bad == 0, decisive_bad, 0)
            else:
                record(checks, "dense_vs_strided", "placement_change_available", "WARN",
                       False, "empty", "dense and strided winner tables available")
        else:
            record(checks, "dense_vs_strided", "placement_change_output_present", "WARN",
                   False, "missing", str(layout_placement_path))
    else:
        record(checks, "dense_vs_strided", "comparison_outputs_present", "WARN",
               False, "missing", "optional when dense GEMM results exist")
    ...
    report = f"""# Independent integrity audit of the combined STRIDED_GEMM outputs
...
"""
    ...
    print(f"[ALL STRIDED_GEMM] integrity audit: {verdict}")
