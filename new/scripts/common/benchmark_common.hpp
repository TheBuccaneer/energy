...
inline const std::vector<int> THREAD_COUNTS{1, 2, 4, 8, 10, 16, 20};
...
public:
    Rapl() {
        ...
        // /sys/class/powercap exposes RAPL zones as symlinks. A recursive
        // iterator does not descend into directory symlinks by default, so it
        // can see the package zone but miss its DRAM sub-zone. Scan the class
        // entries directly (plus one child level) and canonicalize them to
        // avoid duplicate aliases and symlink loops.
    ...
    [[nodiscard]] bool available() const { return !packages_.empty(); }
    [[nodiscard]] bool dram_available() const { return !drams_.empty(); }

    [[nodiscard]] EnergySample read() const {
        return {read_group(packages_), read_group(drams_)};
    }

    [[nodiscard]] EnergyDelta delta(const EnergySample& before, const EnergySample& after) const {
        return {
...
inline void require_rapl(const Rapl& rapl) {
    ...
    throw std::runtime_error(
        "No readable RAPL package energy_uj zone. Run 01_enable_CPU_Intel.sh or use sudo.");
}
