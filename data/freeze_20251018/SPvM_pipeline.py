...
FIXED: Now includes matrix_size column in output!
"""
...
wanted = [
    "timestamp", "host", "gpu_name", "cpu_model", "matrix_size",  # <-- FIXED: Added matrix_size
...
for p in sorted(files):
    ...
    numeric_cols = [
        ...
        "matrix_size"  # <-- FIXED: Added to numeric conversion
    ]
    ...
    if 'matrix_size' in df.columns:
        valid_size = df['matrix_size'].notna().sum()
        unique_sizes = df['matrix_size'].nunique()
        print(f"  ✅ Valid matrix_size values: {valid_size}/{len(df)} ({unique_sizes} unique sizes)")
    
    # Check effective_size
    if 'effective_size' in df.columns:
        valid_eff = df['effective_size'].notna().sum()
        unique_eff = df['effective_size'].nunique()
        print(f"  ✅ Valid effective_size values: {valid_eff}/{len(df)} ({unique_eff} unique sizes)")
    ...
    # Calculate effective matrix size (N²)
    # GPU: matrix_size * matrix_size
    # CPU: nrows * nrows (for square matrices)
    def calc_effective_size(row):
        if pd.notna(row.get("matrix_size")):
            return row["matrix_size"] * row["matrix_size"]
        elif pd.notna(row.get("nrows")):
            return row["nrows"] * row["nrows"]
        else:
            return None
    
    df["effective_size"] = df.apply(calc_effective_size, axis=1)
    ...
    if matrix_id_col:
        # Include matrix_size/nrows in grouping to get separate rows per size!
        size_group_col = None
        if "matrix_size" in df.columns and df["matrix_size"].notna().any():
            size_group_col = "matrix_size"
        elif "nrows" in df.columns and df["nrows"].notna().any():
            size_group_col = "nrows"
        
        group_cols_matrix = ["setup_id", "cpu_type", matrix_id_col]
        if size_group_col:
            group_cols_matrix.append(size_group_col)
        group_cols_matrix.extend(["gpu_name", "pcie_gen_current", "threads", "impl", "dtype"])
        ...
        # Matrix properties
        if "matrix_size" in df.columns:  # <-- Keep original for reference
            agg_dict_matrix["matrix_size"] = ("matrix_size", "first")
        if "effective_size" in df.columns:  # <-- NEW: Effective size (N²)
            agg_dict_matrix["effective_size"] = ("effective_size", "first")
...
if agg_by_matrix_all:
    ...
    numeric_output_cols = [col for col in final_by_matrix.columns 
                          if any(x in col for x in ['eur_job', 'co2_job', 'kwh', 'seconds', 'gflops', 'price', 'joule', 'bw_gb_s', 'gb_per_s', 'j_per_gb', 'passes', 'nrows', 'ncols', 'nnz', 'avg_nnz', 'matrix_size', 'effective_size'])]  # <-- FIXED: Added effective_size
    for col in numeric_output_cols:
    ...
    # Print matrix_size info
    if "matrix_size" in final_by_matrix.columns:
        unique_sizes = final_by_matrix['matrix_size'].dropna().unique()
        print(f"  📏 Matrix sizes (N) found: {sorted([int(s) for s in unique_sizes])}")
    
    if "effective_size" in final_by_matrix.columns:
        unique_eff = final_by_matrix['effective_size'].dropna().unique()
        print(f"  📏 Effective sizes (N²) found: {sorted([int(s) for s in unique_eff])}")
