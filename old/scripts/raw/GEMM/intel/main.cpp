...
int determineBatchSize(float* h_A, float* h_B, float* h_C, int n, int lda, double target_time) {
    (void)target_time;  // Unused, kept for API compatibility
    const float alpha = 1.0f;
    const float beta = 0.0f;
    const double min_time = 1.0;  // Minimum for stable measurement
    const double max_time = 3.0;  // Maximum to avoid thermal drift
    
    // Adaptive starting point based on problem size
    // Large N = slow GEMMs → start small
    // Small N = fast GEMMs → start large
    int batches;
    if (n >= 8192) {
        batches = 1;
    } else if (n >= 4096) {
    ...
    // Iteratively scale until we're in the target window
    while (batches <= MAX_BATCH_SIZE) {
        ...
        if (measured_time >= min_time) {
            ...
            int scaled_batches = static_cast<int>(batches * (max_time / measured_time));
        ...
        int needed_batches = static_cast<int>(std::ceil(min_time / time_per_batch));
