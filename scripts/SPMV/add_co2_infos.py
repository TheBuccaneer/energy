...
def enrich_measurement_file(file_path, ci_df, issues_log, qa_log):
    ...
    try:
        ...
        # - Standard: batches, kWh_batch
        # - Alternative: batches, kWh_per_pass
        
        # Detect which convention is used
        has_batch = 'kWh_batch' in df.columns
        has_per_pass = 'kWh_per_pass' in df.columns
        
        if ('batches' in df.columns) and (has_batch or has_per_pass):
            df['batches'] = pd.to_numeric(df['batches'], errors='coerce')
            
            # Select the correct kWh column
            if has_per_pass:
                kwh_col = 'kWh_per_pass'
                print(f"  Using 'per_pass' naming convention")
            else:
                kwh_col = 'kWh_batch'
                print(f"  Using 'batch' naming convention")
            
            # Handle comma-decimal if it's a string
            if df[kwh_col].dtype == 'object':
                df[kwh_col] = df[kwh_col].astype(str).str.replace(',', '.', regex=False)
            df[kwh_col] = pd.to_numeric(df[kwh_col], errors='coerce')
            
            # Calculate batch/pass CO₂ for all areas
            for area in ['de', 'fr', 'pl']:
                # Use a generic internal name for calculation
                co2_per_unit = df[kwh_col] * df[f'{area}_ci_g_per_kwh'] / 1000.0
                
                # Store with appropriate naming convention
                if has_per_pass:
                    df[f'{area}_co2_per_pass_kg'] = co2_per_unit
                    df[f'{area}_co2_job_from_passes_kg'] = co2_per_unit * df['batches']
                else:
                    df[f'{area}_co2_batch_kg'] = co2_per_unit
                    df[f'{area}_co2_job_from_batches_kg'] = co2_per_unit * df['batches']
            
            # QA: check ±1% tolerance
            for area in ['de', 'fr', 'pl']:
                if has_per_pass:
                    co2_from_units = f'{area}_co2_job_from_passes_kg'
                else:
                    co2_from_units = f'{area}_co2_job_from_batches_kg'
                
                mismatch = df[
                    (df[f'{area}_co2_job_kg'].notna()) & 
                    (df[co2_from_units].notna()) &
                    (np.abs((df[f'{area}_co2_job_kg'] - df[co2_from_units]) 
                            / df[f'{area}_co2_job_kg']) > 0.01)
                ]
                if not mismatch.empty:
                    qa_log.write(f"{file_path.name} ({area.upper()}): {len(mismatch)} rows with >1% job/unit CO₂ mismatch\n")
...
def main():
    ...
    # Open log files with UTF-8 encoding (for CO₂ symbol)
    issues_log = open(root / "co2_join_issues.log", "w", encoding='utf-8')
    qa_log = open(root / "co2_qa_warnings.log", "w", encoding='utf-8')
