...
def enrich_measurement_file(file_path, price_df, price_sync_dict, inplace, issues_log, qa_log):
    ...
    try:
        ...
        # === STEP 5: Pass metrics (SpMV uses 'passes' not 'batches') ===
        # In SpMV: 'batches' is just a counter/ID, 'passes' is the actual repetition count
        if 'passes' in df.columns:
            df['passes'] = pd.to_numeric(df['passes'], errors='coerce')
            
            # Calculate kWh per pass if not already present
            if 'kWh_pass' not in df.columns:
                df['kWh_pass'] = df['energy_j'] / df['passes'] / 3.6e6
            else:
                if df['kWh_pass'].dtype == 'object':
                    df['kWh_pass'] = df['kWh_pass'].astype(str).str.replace(',', '.', regex=False)
                df['kWh_pass'] = pd.to_numeric(df['kWh_pass'], errors='coerce')
            
            # Calculate pass costs for all areas
            for area in ['DE', 'FR', 'PL']:
                area_lower = area.lower()
                df[f'{area_lower}_price_eur_kwh_pass'] = df[f'{area_lower}_price_eur_kwh_run']
                df[f'{area_lower}_eur_pass'] = df['kWh_pass'] * df[f'{area_lower}_price_eur_kwh_pass']
                df[f'{area_lower}_eur_pass_sync'] = df['kWh_pass'] * price_sync_dict[area]
                df[f'{area_lower}_eur_job_from_passes'] = df[f'{area_lower}_eur_pass'] * df['passes']
