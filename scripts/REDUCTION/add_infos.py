#!/usr/bin/env python3
"""
Day-Ahead Price Enricher for September 2024 measurements.
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="Enrich measurement CSVs with Day-Ahead prices (Sept 2024)"
    )
...
def load_price_data(price_file):
    ...
    - daily_mean_de, daily_mean_fr, daily_mean_pl
    ...
    try:
        ...
        # Extract sync prices (same in all rows, just take first)
        price_sync = {
        ...
        # Validate: check for NaN prices
        for area in ['de', 'fr', 'pl']:
            col = f'price_eur_kwh_{area}'
            na_count = df[col].isna().sum()
            if na_count > 0:
                print(f"⚠ Warning: {na_count} NaN prices in {col}", file=sys.stderr)
...
def calculate_weighted_price(run_start, run_duration_s, area, price_df):
    ...
    # Single hour case: start and end in same clock hour
...
def enrich_measurement_file(file_path, price_df, price_sync_dict, inplace, issues_log, qa_log):
    ...
    1. Parse timestamps (Europe/Berlin timezone)
    2. Map area (DE/FR/PL)
    ...
    try:
        ...
        # === PARSE TIMESTAMPS & CONVERT 2025->2024, OCT->SEPT ===
        print(f"  Parsing timestamps & converting to Sept 2024...", end='')
        df['timestamp'] = pd.to_datetime(df['timestamp'], errors='coerce')
        
        # Make timezone-aware (Europe/Berlin)
        tz = ZoneInfo('Europe/Berlin')
        if df['timestamp'].dt.tz is None:
            df['timestamp'] = df['timestamp'].dt.tz_localize(tz)
        else:
        ...
        # Convert 2025 -> 2024
        mask_2025 = df['timestamp'].dt.year == 2025
        df.loc[mask_2025, 'timestamp'] = df.loc[mask_2025, 'timestamp'].apply(
            lambda dt: dt.replace(year=2024) if pd.notna(dt) else pd.NaT
        )
        
        # Convert October -> September (reject Oct 31 - Sept has only 30 days)
        mask_oct = df['timestamp'].dt.month == 10
        mask_oct31 = mask_oct & (df['timestamp'].dt.day == 31)
        
        # Reject Oct 31 timestamps
        df.loc[mask_oct31, 'timestamp'] = pd.NaT
        
        # Convert Oct 1-30 -> Sept 1-30
        mask_oct_valid = mask_oct & ~mask_oct31
        df.loc[mask_oct_valid, 'timestamp'] = df.loc[mask_oct_valid, 'timestamp'].apply(
            lambda dt: dt.replace(month=9) if pd.notna(dt) else pd.NaT
        )
        ...
        # === CALCULATE PRICES FOR ALL AREAS ===
...
def main():
    ...
    csv_files = [f for f in root.rglob("*.csv") 
                 if f != price_file and 'day_ahead' not in f.name.lower() 
